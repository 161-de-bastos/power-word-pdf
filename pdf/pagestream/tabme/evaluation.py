...
def predict (path_data, path_model_folder, path_csv='results', batch_size=12, path_cache_folder=None, num_hidden_features=None, ablation=None):
    ...
    tabme = TABME(path_data=path_data, path_cache_folder=path_cache_folder, ablation=ablation, max_seq_length=max_seq_length, input_img_size=input_img_size)
    ...
    model.load_state_dict(torch.load(path_model_folder/"best"/"model_weights.pt", map_location=device),strict=False)
    model = model.to(device)
    model.eval()

    dataloader = DataLoader(tabme, batch_size = batch_size, drop_last = False)
    ...
    df = pd.DataFrame(all_logits_dict).T.reset_index()
    df.columns = ['stem','logit_0','logit_1']
    df['prediction'] = df['stem'].map(lambda stem: np.argmax(all_logits_dict[stem]))

    if ablation == 'resnet':
        df.to_csv(f'{path_csv}_minus_resnet.csv')
    elif ablation == 'layoutlm':
        df.to_csv(f'{path_csv}_minus_layoutlm.csv')
    else: 
