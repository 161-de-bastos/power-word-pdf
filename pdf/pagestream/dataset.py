...
class TABME(Dataset):
    ...
    '''
    def __init__(self, path_data: str, path_cache_folder: str=None, ablation=None, max_seq_length: int =100, input_img_size: tuple=(1000,1000)):
        ...
        page_ids = [path.stem for path in all_path_tsv]
        idx = np.argsort([
            int(main) * 100000 + int(sub) for main, sub in re.findall(r'(\d+)-(\d+)', ' '.join(page_ids))
        ])
        self.page_ids = np.asarray(page_ids)[idx]
        ...
        if self.path_cache_folder: # self.path_cache_folder != None
            ...
            if not self.path_cache.exists():
                ...
                for page_id in tqdm(self.page_ids):
                    ...
                    path_tsv = self.path_data/f"{page_id}.tsv"
                    ...
                    # image
                    path_img = self.path_data/f"{page_id}.jpg"
    ...
    def __getitem__(self, idx):
        ...
        return ocr_input, image, page_id
